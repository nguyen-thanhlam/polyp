...
## **MÔ HÌNH NSAST VÀ 4 YẾU TỐ**

Điểm AUC và khoảng tin cậy 95%
```{r auc, echo = F, eval = T}
auc(roc3)
...
Điểm cut-off value của mô hình sử dụng (cái này chưa ra)

```{r, echo = F, eval = T}
#coords(roc3, x = "best", best.method="youden", ret = c("threshold", "sensitivity", "specificity", "npv", "ppv")) 
```

P-value theo Hosmer-Lemeshow
```{r p, echo = F, eval = T}
test$pred <- predict(mod3, test, type = "response")
a <- data.frame(test$nhompolyp, test$pred)
a <- na.omit(a)
hoslem.test(a$test.nhompolyp, a$test.pred, g = 10)
```

**Điểm C-index của nhóm training**
...
**Điểm C-index của nhóm validation**
